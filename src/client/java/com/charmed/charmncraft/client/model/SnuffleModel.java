...
public class SnuffleModel<T extends SnuffleEntity> extends AnimalModel<T> {
    ...
    @Override
    public void animateModel(SnuffleEntity snuffle, float limbSwing, float limbSwingAmount, float partialTick) {
        boolean flag = !snuffle.isBaby();
        this.horns.visible = flag;
        this.hump.visible = flag; // Always show hump (non-legacy mode)
    }

    @Override
    public void setAngles(SnuffleEntity snuffle, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
