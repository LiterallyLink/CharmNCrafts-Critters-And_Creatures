package com.charmed.charmncraft.block;
...
public class FrostyFluffBlock extends Block {
        ...
        if (entity instanceof LivingEntity && !entity.bypassesSteppingEffects() && !world.isClient()) {
            if (entity.lastRenderX != entity.getX() || entity.lastRenderY != entity.getY() || entity.lastRenderZ != entity.getZ()) {
                if (world.getRandom().nextInt(5) == 0) {
                    ((ServerWorld) world).spawnParticles(ModParticles.SNOWFLAKE, entity.getX(), entity.getY(), entity.getZ(), 0, MathHelper.nextBetween(world.getRandom(), -1.0F, 1.0F) * 0.083F, 0.05F, MathHelper.nextBetween(world.getRandom(), -1.0F, 1.0F) * 0.083F, 1.0F);
                }
            }
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hitResult) {
        ...
        if (stack.isOf(Items.MAGMA_CREAM)) {
            if (!world.isClient()) {
                ...
                world.setBlockState(pos, ModBlocks.SNUFFLE_FLUFF.getDefaultState());
                world.playSound(null, pos, ModSounds.ENTITY_SNUFFLE_THAW, SoundCategory.BLOCKS, 0.7F, 1.6F + (world.random.nextFloat() - world.random.nextFloat()) * 0.4F);
            }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if (!world.getBlockState(pos.down()).isOpaque())
            world.addParticle(ModParticles.SNOWFLAKE, pos.getX() + random.nextDouble(), pos.getY() - 0.1F, pos.getZ() + random.nextDouble(), 0.0F, 0.0F, 0.0F);
    }
}
